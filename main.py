...
def init_mask(mask, triples, entity2id, relation2id, relation_tail):
    ...
    for h, r, t, x_hat, y_hat in triples:
        if t not in relation_tail: continue
        # if y_hat not in relation_tail: continue
        entity_a = entity2id[h]
        entity_b = relation_tail[t]
        relation = relation2id[r]
...
def load_data(data_path, target_relation):
    ...
    with open(data_path, mode='r') as fd:
        for line in fd:
            ...
            data_ori.append((h, r, t))
            data_inv.append((t, 'INV' + r, h))
...
def valid_process(valid_data, model, train_kg, flag, batch_size, entity2id,
                  relation2id, id2entity, graph_entity, option, raw=False):
    if len(valid_data) == 0: return 0, 0, 0, 0
    model.eval()
    i, v = get_init_matrix(dataset.kg, dataset.entity2id, dataset.relation2id, [])
    matrix_all = torch.sparse.FloatTensor(i, v,
                                          torch.Size([len(dataset.entity2id) * (len(dataset.relation2id) + 1),
                                                      len(dataset.entity2id)]))
    id2tail = {}
    tail2id = {}
    for h, r, t in valid_data:
        if t not in tail2id:
            tail2id[t] = len(tail2id)
            id2tail[tail2id[t]] = t
    if len(tail2id) == 0: return
    ...
    for i in range(batch_num):
        ...
        for step in range(option.step):
            state = model(matrix, matrix_all, all_states, step, entity2id_tail, flag)
            # print(state.sum())
    ...
    for h, r, t in valid_data:
        ...
        truth_score_ori = total_states[entity2id[h]][tail2id[t]]
        index_head = torch.argsort(total_states[:, tail2id[t]], descending=True).cpu().numpy()
        if raw:
            rank = torch.sum((total_states[:, tail2id[t]] >= truth_score_ori).int())
        else:
            scores_head = total_states[:, tail2id[t]].clone()
            for tail in graph_entity[t]:
                ...
                if r_tmp.startswith('INV'):
                    ...
                    r_tmp = 'INV' + r_tmp
                if r_tmp in graph_entity[t][tail]: scores_head[entity2id[tail]] = -1e20
        ...
        if rank <= 10:
            hit_10 += 1
        # print((h, r, t), truth_score_ori, rank_head)
        count += 1
    if count > 0:
        mrr /= count
        hit_1 /= count
...
def main(dataset, valid_data, test_data, option):
    ...
    model = ICLMModel(len(dataset.relation2id), option.step, option.length,
                           len(dataset.entity2id), option.tau_1, option.tau_2, option.use_gpu)
    ...
    for e in range(option.max_epoch):
        ...
        for k, batch in enumerate(dataset.batch_iter()):
            ...
            i, v = get_init_matrix(dataset.kg, dataset.entity2id, dataset.relation2id, triples)
            matrix_all = torch.sparse.FloatTensor(i, v,
                                                  torch.Size([len(dataset.entity2id) * (len(dataset.relation2id) + 1),
                                                              len(dataset.entity2id)]))
            matrix = torch.zeros([len(dataset.entity2id), len(entity_tail), len(dataset.relation2id) + 1])
            mask = torch.zeros([len(dataset.entity2id), len(entity_tail), len(dataset.relation2id) + 1])
            init_matrix(matrix, dataset.kg, dataset.entity2id, dataset.relation2id, entity_tail)
            init_mask(mask, triples, dataset.entity2id, dataset.relation2id, entity_tail)
            matrix = matrix * (1 - mask)
            matrix = matrix.view(-1, len(dataset.relation2id) + 1).to_sparse()
            if option.use_gpu:
                matrix = matrix.cuda()
                matrix_all = matrix_all.cuda()
            all_states = []
            for t in range(option.step):
                state = model(matrix, matrix_all, all_states, t, entity_tail, flag, 1 - (e + 1) / option.max_epoch, is_training=True)
        ...
        if option.early_stop:
            mrr_ori, hit_1_ori, hit_3_ori, hit_10_ori = valid_process(valid_data[0], model, dataset.kg, False,
                                                      option.batch_size, dataset.entity2id, dataset.relation2id,
                                                      dataset.id2entity, graph_entity, option)
            mrr_inv, hit_1_inv, hit_3_inv, hit_10_inv = valid_process(valid_data[1], model, dataset.kg, True,
                                                                      option.batch_size, dataset.entity2id,
                                                                      dataset.relation2id,
                                                                      dataset.id2entity, graph_entity, option)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lammda', default=0.01, type=float)
