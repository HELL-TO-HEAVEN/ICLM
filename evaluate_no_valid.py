...
def load_kg_form_pkl(file_path):
    ...
    with open(file_path + '/tail2id_{}.pkl'.format('ori'), mode='rb') as fd:
        tail2id_ori = pkl.load(fd)
    with open(file_path + '/tail2id_{}.pkl'.format('inv'), mode='rb') as fd:
        tail2id_inv = pkl.load(fd)
    return kg, entity2id, relation2id, tail2id_ori, tail2id_inv
...
def extend_graph(graph_entity, test_data):
...
def evaluate(id2entity, entity2id, id2relation, relation2id, tail2id_ori, tail2id_inv, train_kg,
             test_data, target_relation):
    ...
    extend_graph(graph_entity, test_data[0])
    extend_graph(graph_entity, test_data[1])
    ...
    for h, r, t in tqdm(test_data[0]):
        ...
        if h not in entity2id or t not in tail2id_ori: continue
        truth_score_ori = total_states_ori[entity2id[h]][tail2id_ori[t]]
        if raw:
            rank_head = torch.sum((total_states_ori[:, tail2id_ori[t]] >= truth_score_ori).int())
        else:
            scores_head = total_states_ori[:, tail2id_ori[t]].clone()
            ...
            rank_head = m + (n + 1) / 2
            # print((h, r, t), m, n, rank_head, truth_score_ori)
        ...
        if rank_head <= 10:
            ...
            hit_10_head += 0
        # print((h, r, t), truth_score, rank_head, rank_tail)
        count_head += 1

    for h, r, t in tqdm(test_data[1]):
        if r.replace('/', '|') != 'INV' + target_relation: continue
        if h not in entity2id or t not in tail2id_inv: continue
        truth_score_inv = total_states_inv[entity2id[h]][tail2id_inv[t]]
        if raw:
            rank_tail = torch.sum((total_states_inv[:, tail2id_inv[t]] >= truth_score_inv).int())
        else:
            scores_tail = total_states_inv[:, tail2id_inv[t]].clone()
            for tail in graph_entity[t]:
                if tail not in entity2id: continue
                r_tmp = r
                if r_tmp.startswith('INV'):
                    ...
                    r_tmp = 'INV' + r_tmp
                if r_tmp in graph_entity[t][tail]: scores_tail[entity2id[tail]] = -1e20
            n = torch.sum((scores_tail == truth_score_inv).int()) + 1
            m = torch.sum((scores_tail > truth_score_inv).int())
            rank_tail = m + (n + 1) / 2
            # print((h, r, t), m, n, rank_tail, truth_score_inv)
...
if __name__ == '__main__':
    ...
    if 'family_dual' in data_dir: target_file = 'relations.txt'
    ...
    with open('{}/{}'.format(data_dir, target_file), mode='r') as fd:
        for line in tqdm(fd.readlines()):
            ...
            train_kg, entity2id, relation2id, tail2id_ori, tail2id_inv = load_kg_form_pkl(option.exp_dir)
            ...
            hit_10_head, hit_10_tail, count_head, count_tail = evaluate(id2entity, entity2id, id2relation,
                                                       relation2id, tail2id_ori, tail2id_inv, train_kg,
